...
在调用任何 UI/UX Pro Max 或 Impeccable 指令前，AI **必须**先确认依赖是否真实存在，**不得假设已安装**：

1. 检查文件是否存在：`~/.claude/skills/ui-ux-pro-max/scripts/search.py`
2. 检查 Impeccable 指令（`/arrange`、`/typeset`、`/colorize`、`/delight`、`/polish`、`/critique`）是否在当前环境可用。

根据探测结果走对应分支：

- **全部可用** → 按 4.2.1 ~ 4.2.5 执行完整两阶段设计流程。
- **部分 / 全部缺失** → 进入【降级模式】，**不要**报错卡住，也**不要**伪造脚本输出。改为告知用户：
  > ⚙️ 未检测到 UI/UX Pro Max / Impeccable Skills，已切换为「基础设计规范」生成原型：
  > 采用 Tailwind 默认色板 + 8px 间距栅格 + 单一主色 + 系统字体栈，遵循对比度、留白、视觉层级的通用原则。
  > 如需专业级设计系统，可单独安装 ui-ux-pro-max 后重跑步骤四。

  然后直接用基础规范产出原型，跳过所有 `search.py` 调用与 Impeccable 指令。
...
#### 4.2.5 第二阶段：Impeccable Skills 前端设计打磨（推荐执行）

如果选择使用设计系统，在基于设计系统完成 HTML 原型初稿后，**推荐调用 Impeccable Skills** 进行专业级打磨，确保视觉细节和交互体验达到专业水准。以下是在原型生成后推荐依次调用的指令：
...
**跳过说明**：如果用户已有设计规范或处于快速模式，可直接使用 Tailwind CSS 快速搭建原型，无需执行上述指令。
