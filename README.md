...
## 🛠️ Trae Skill 如何安装与使用？

如果你使用的是 **Trae IDE**，我们已经为你封装好了专属的 Skill。安装后只需输入 `/agile-pm-workflow` 即可一键开启！

### 安装步骤：

1. 下载或 `git clone` 本仓库到你的本地电脑。
2. 找到本仓库中的 `agile-pm-workflow_skill` 文件夹。
3. 将该文件夹复制并重命名为你本地 Trae 的全局技能目录：
   - **Windows**: `C:\Users\你的用户名\.trae\skills\agile-pm-workflow`
   - **Mac/Linux**: `~/.trae/skills/agile-pm-workflow`
   *(注意：如果没有 `skills` 文件夹，请手动新建一个)*
4. 重新启动 Trae IDE。

### 使用方法：

在 Trae 的对话框中直接输入 `/agile-pm-workflow` 并发送，然后告诉它你的新项目想法。AI 就会自动帮你建好标准的文件夹结构，并开始一步步引导你产出专业的需求文档了！
